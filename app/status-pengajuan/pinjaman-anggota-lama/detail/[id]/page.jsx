...
const DetailPinjamanAnggotaBaru = () => {
  ...
  const verifyData = async () => {
    setLoading(true);
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}loans/${
        member.deposit.loans.find((loan) => !loan.verified).id
      }/verify`,
      {
        method: "POST",
        headers: {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            disabled={
              member.deposit.loans.find((loan) => !loan.verified)?.status !==
              "disetujui"
            }
